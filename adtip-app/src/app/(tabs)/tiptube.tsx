...
export default function TipTubeScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const premiumPosts = Array.isArray(premiumData)
        ? premiumData
        : premiumData.status === 200 && premiumData.data
          ? premiumData.data
          : [];

      const publicVideosResponse = await fetch(
        `${API_URL}/api/getpublicvideos/0/2`,
      );
      if (!publicVideosResponse.ok) {
        throw new Error(
          `Public videos HTTP error! status: ${publicVideosResponse.status}`,
        );
      }
      const publicVideosData = await publicVideosResponse.json();
      const publicVideos = Array.isArray(publicVideosData)
        ? publicVideosData
        : publicVideosData.status === 200 && publicVideosData.data
          ? publicVideosData.data
          : [];

      const publicShotsResponse = await fetch(`${API_URL}/api/getpublicshots`);
      if (!publicShotsResponse.ok) {
        throw new Error(
          `Public shots HTTP error! status: ${publicShotsResponse.status}`,
        );
      }
      const publicShotsData = await publicShotsResponse.json();
      const publicShots = Array.isArray(publicShotsData)
        ? publicShotsData
        : publicShotsData.status === 200 && publicShotsData.data
          ? publicShotsData.data
          : [];

      const mapToVideoCard = (
        post: PremiumPost | PublicVideo | PublicShot,
      ): VideoCardVideo => {
        let thumbnail: string | null = null;
        if (
          post.video_Thumbnail &&
          post.video_Thumbnail !== 'undefined' &&
          post.video_Thumbnail.trim() !== ''
        ) {
        ...
        return {
          ...
          channel: {
            ...
            avatar:
              post.channel_profile && post.channel_profile !== 'null'
                ? post.channel_profile
          ...
          description:
            post.video_description && post.video_description !== 'undefined'
        ...
        <>
          <FlatList
            ...
            keyExtractor={(item, index) =>
              'type' in item
                ? `shorts-${index}`
                : (item as VideoCardVideo).id.toString()
            }
