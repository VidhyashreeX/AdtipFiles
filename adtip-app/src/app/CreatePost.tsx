...
export default function CreatePostScreen() {
  ...
  const handleNext = () => {
    if (!selectedMedia || !caption.trim()) {
      Alert.alert(
        'Error',
        'Please select an image or video and add a caption before proceeding.',
      );
      return;
    }
    router.push({
  ...
  const requestMediaLibraryPermission = async (): Promise<boolean> => {
    try {
      ...
      Alert.alert(
        'Error',
        'Failed to request media library permission. Please try again.',
      );
      return false;
    }
  };
  ...
  const availableHeight =
    screenHeight - headerHeight - footerHeight - insets.top - insets.bottom;
