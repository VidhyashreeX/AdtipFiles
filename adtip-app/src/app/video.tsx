...
export default function VideoPlayerScreen() {
  ...
  const parsedVideo: VideoCardVideo | null = video
    ? JSON.parse(video as string)
    : null;
  ...
  const fetchRecommendedVideos = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/getpublicvideos/0/10?category=${parsedVideo.category}`,
      );
      if (!response.ok) {
      ...
      const mapToVideoCard = (post: any): VideoCardVideo => {
        ...
        console.log(
          `Mapping thumbnail for recommended post ${post.id}:`,
          thumbnail,
        );
      ...
      <View style={styles.videoContainer}>
        ...
        <View style={styles.videoInfo}>
          ...
          <Text style={styles.meta}>
            {parsedVideo.views} views •{' '}
            {new Date(parsedVideo.createdAt).toLocaleDateString()}
          </Text>
