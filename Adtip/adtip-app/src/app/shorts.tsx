...
export default function ShortsScreen() {
  ...
  const fetchShorts = async () => {
    try {
      ...
      const publicShots = Array.isArray(data) ? data : data.status === 200 && data.data ? data.data : [];

      const mappedShorts: ShortVideo[] = publicShots.map((shot: PublicShot) => ({
        id: shot.id?.toString() || 'unknown',
        title: shot.name || 'Untitled Short',
        thumbnail: shot.video_Thumbnail && shot.video_Thumbnail !== 'undefined' ? shot.video_Thumbnail : 'https://via.placeholder.com/150',
        channel: {
          id: shot.channelId?.toString() || 'unknown',
          name: shot.channelName || 'Unknown Channel',
          avatar: shot.channel_profile && shot.channel_profile !== 'null' ? shot.channel_profile : 'https://via.placeholder.com/36',
          verified: false,
        ...
        description: shot.video_description && shot.video_description !== 'undefined' ? shot.video_description : 'No description available',
  ...
  return (
    <View style={styles.container}>
      ...
      <SafeAreaView style={[styles.header, { height: HEADER_HEIGHT }]} edges={['top']}>
