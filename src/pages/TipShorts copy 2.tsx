...
const TipShorts = () => {
    ...
    if (video) {
      ...
      video.play()
        ...
        .catch((e) => {
          ...
          // If autoplay fails, mute it and keep it paused or try playing muted
          video.muted = true;
          setIsMuted((prev) => ({ ...prev, [id]: true }));
          setIsPlaying((prev) => ({ ...prev, [id]: false }));
          // Optionally, you could still try to play muted here:
          // video.play().then(() => setIsPlaying((prev) => ({ ...prev, [id]: true })));
        });
    }
  }, []);
  ...
  const pauseVideo = useCallback((id: number) => {
    ...
    if (video) {
      video.pause();
      video.muted = true; // Always mute when paused
      setIsPlaying((prev) => ({ ...prev, [id]: false }));
      setIsMuted((prev) => ({ ...prev, [id]: true }));
    }
  }, []);
  ...
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting && entry.intersectionRatio >= 0.85) {
            // This video is now mostly in view
            setCurrentIndex(shortIndex);
            playVideo(videoId);
            preloadNext(shortIndex); // Preload *after* the current video starts
          } else {
            // This video is no longer the primary focus
            pauseVideo(videoId);
          }
        });
      },
      { threshold: 0.85 } // Trigger when 85% of the video is visible
    );
    ...
    // Ensure the first video plays on initial load if available
    if (shorts[0] && videoRefs.current.get(shorts[0].id)) {
  ...
  // --- Manual Navigation (for buttons) ---
  const goTo = (dir: "next" | "prev") => {
    let newIdx = currentIndex;
    if (dir === "next" && currentIndex < shorts.length - 1) newIdx++;
    if (dir === "prev" && currentIndex > 0) newIdx--;
    setCurrentIndex(newIdx);
    // Manually scroll into view if navigation buttons are used
    const videoElement = videoRefs.current.get(shorts[newIdx]?.id);
    if (videoElement) {
            ...
            >
              ...
              >
                <video
                  ...
                  loop
                  playsInline
                  muted={isMuted[short.id] ?? true} // Default to muted
